...
void saveToHistory(char* command) {
    ...
    strncpy(historyBufferArray.bufferArray[historyBufferArray.lastCommandIndex], command, INPUT_BUF);
    historyBufferArray.lengthArray[historyBufferArray.lastCommandIndex] = strlen(command);
...
void printHistory(void) {
    ...
    for (i = 0; i <= historyBufferArray.numOfCommandsInMem; i++) {
        cprintf("Command %d: %s\n", i + 1, historyBufferArray.bufferArray[i]);
    }
}
...
void printSortedHistory(void) {
    int i;
    for (i = 0; i < historyBufferArray.numOfCommandsInMem ; i++) {
        cprintf("%s", historyBufferArray.bufferArray[(historyBufferArray.lastCommandIndex - i + MAX_HISTORY) % MAX_HISTORY]);
    }
}
...
int history(int historyId) {
    ...
    if (historyId < 0 || historyId > historyBufferArray.numOfCommandsInMem-1) {
        cprintf("Error: Invalid historyId. It should be between 0 and %d\n", historyBufferArray.numOfCommandsInMem-1);
        release(&history_lock);
        return -1;
    }
    ...
    int index = (historyBufferArray.lastCommandIndex - historyId + MAX_HISTORY) % MAX_HISTORY;
